...
# Page configuration name (Tab name when opened on browser)
st.set_page_config("Linear Regresson", layout = "centered")

#Load CSS
def load_css(file):
    with open(file) as f:
        #streamlit does not accept html by default hence we explicitly declare unsafe allow html as true, by default it's false
...
# Load Data
@st.cache_data() #helps in storing the data in cache, no need to load data everytime.
def load_data():
...
# Calculation coefficient and intercept
